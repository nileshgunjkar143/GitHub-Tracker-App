...
type PropsType = {
  voilatorsData: IVoilatorsAndVoilationsFixedType;
  currentYear: number;
  theme: themeType;
};

function VoilatorsCard({voilatorsData, currentYear, theme}: PropsType) {
        ...
        <DataTable.Cell>
          <Card.Content style={styles.cardContentStatusStyle}>
            <Paragraph style={[styles.forYear, {color: theme.textColor}]}>
              ...
              {voilatorsData?.total_violators_fixed} Fixed ({currentYear})
            </Paragraph>
            <Paragraph style={[styles.vforMonth, {color: theme.textColor}]}>
              <AntDesign color="green" size={20} name="arrowup" />{' '}
              {voilatorsData?.max_violators_count} Voilated ({currentYear})
            </Paragraph>
          </Card.Content>
        </DataTable.Cell>
      </DataTable.Row>
